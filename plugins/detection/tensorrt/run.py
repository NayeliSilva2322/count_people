"""Detection plugin — TensorRT edition (PAID).

    python plugins/detection/tensorrt/run.py --ipc cam0 --imshow
    python plugins/detection/tensorrt/run.py --source 0 --model models/detection/yolo26s.engine --imshow
"""
...
from detect import TensorRTDetector  # noqa: E402
...
def main() -> None:
    """CLI entry for the TensorRT (paid) detection plugin."""
    parser = argparse.ArgumentParser(description="plugins/detection tensorrt (paid)")
    ...
    device = str(run.get("device") or "0")
    ipc = run.get("ipc")
    ...
    if source is None or str(source) == "":
        raise SystemExit(
            ...
            "  python plugins/detection/tensorrt/run.py --ipc cam0 --imshow\n"
            "  python plugins/detection/tensorrt/run.py --source 0 --model model.engine --imshow"
        )
    model = run.get("model")
    if not model:
        raise SystemExit("TensorRT standalone requires --model path/to/model.engine")
